namespace AI {
	...
	void SquaredGrid<CoordinateType, PriorityType>::Initialize(CoordinateType width, CoordinateType height)
	{
		// Clear old grid
		m_grid.clear();
	...
	template <typename CoordinateType, typename PriorityType>
	void SquaredGrid<CoordinateType, PriorityType>::Precompute(void)
	{
	...
	template <typename CoordinateType, typename PriorityType>
	void SquaredGrid<CoordinateType, PriorityType>::GetNeighbors(const TNode & current, std::vector <TNode> & neighbors) const
	{
		for (int nDir = 0; nDir < 4; nDir++)
		{
			int aX = current.X() + directions[nDir].dx;
	...
	/// \param  neighbors The vector where to store all neighbors
	template <typename CoordinateType, typename PriorityType>
	void SquaredGrid<CoordinateType, PriorityType>::GetComputedNeighbors(const TNode & current, std::vector < TNode > & neighbors) const
	{
	...
	template <typename CoordinateType, typename PriorityType>
	inline const Node<CoordinateType, PriorityType> & SquaredGrid<CoordinateType, PriorityType>::GetNode(CoordinateType x, CoordinateType y) const
	{
	...
	/// \param  y The Y coordinate of the node
	template <typename CoordinateType, typename PriorityType>
	inline void SquaredGrid<CoordinateType, PriorityType>::SetNodeNonPassable(CoordinateType x, CoordinateType y)
	{
